...
def read_annotations(ann_dir, img_dir, cache_name, labels=[]):
    if os.path.exists(cache_name):
        ...
        for ann in sorted(os.listdir(ann_dir)):
            if '.xml' in ann:
                img = {'object':[]}
                img['filename'] = img_dir + ann[:-3]+'jpg'
...
def correct_yolo_boxes(boxes, image_h, image_w, net_h, net_w):
    if (float(net_w)/image_w) < (float(net_h)/image_h):
        ...
        new_h = net_h
        new_w = net_w#(image_w*net_h)/image_h
...
def preprocess_input(image, net_h, net_w):
    ...
    if (float(net_w)/new_w) < (float(net_h)/new_h):
        new_h = net_h #(new_h * net_w)//new_w
        new_w = net_w
    else:
        new_w = net_w #(new_w * net_h)//new_h
        new_h = net_h
...
def decode_netout(netout, anchors, obj_thresh, net_h, net_w):
    ...
    for i in range(grid_h*grid_w):
        ...
        for b in range(nb_box):
            ...
            box = BoundBox(xmin, ymin, xmax, ymax, objectness, classes)
